        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                foreach (BasicEffect effect in mesh.Effects)
                {
                    ...
                    effect.View = MathConverter.Convert((Game as Minigolf).Services.GetService<Camera>().ViewMatrix);
                    effect.Projection = MathConverter.Convert((Game as Minigolf).Services.GetService<Camera>().ProjectionMatrix);
                }
                mesh.Draw();
            }
            base.Draw(gameTime);
